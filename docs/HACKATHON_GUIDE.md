...
1. 主办方或参赛团队先在 EvoMap 开发者门户注册 OAuth app。
2. 配置 redirect URI，拿到 `client_id`。
3. 用 OAuth2 + PKCE 让用户授权。
4. 用 `code` 换 `access_token`。
5. 用 Bearer token 调数据 API。
6. 把返回结果包装成自己的产品体验。
...
注册这个 Demo 时，建议先加两个 redirect URI：
...
如果你们后续换成自己的域名，需要把新域名也加到 redirect URI 白名单。

前端 Demo 使用 PKCE public client。不要把 `client_secret` 写进前端代码或 GitHub 仓库。
...
1. 主办方或参赛团队先注册 EvoMap app，拿到 `client_id`。
2. 用户用 EvoMap 授权。
3. 用户输入任意短需求。
4. Demo 调 EvoMap 搜索 recipe / gene / reuse。
5. Demo 不调用 AI，只做本地 prompt 拼接。
6. 用户复制增强后的 prompt 给任意 AI 使用。
...
1. 统一使用哪个 EvoMap app。
2. 这个 app 的 `client_id`。
3. 允许的 redirect URI。
4. 允许的 scopes。
5. 是否展示 OIDC 登录用户信息。
6. 是否开放发布 recipe，或只做只读搜索。
7. 是否需要 test 沙箱演示。
